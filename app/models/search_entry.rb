...
# Indexes
#
#  index_search_entries_on_account_id_ngram_submission     (account_id,tsvector) WHERE ((record_type)::text = 'Submission'::text) USING gin
#  index_search_entries_on_account_id_ngram_submitter      (account_id,tsvector) WHERE ((record_type)::text = 'Submitter'::text) USING gin
#  index_search_entries_on_account_id_ngram_template       (account_id,tsvector) WHERE ((record_type)::text = 'Template'::text) USING gin
